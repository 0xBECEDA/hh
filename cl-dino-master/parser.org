...
- список ~y-point~-ов. Это список координат Y image-down, который нужен, чтоб
  накладывать image-down на image-up построчно, пока изображения
  image-down или image-up не кончатся. В случае вопросов по механизму наложения см
  xor-area
...
Для упрощения отладки результаты и таски будут представлены структурами. А храниться
они будут в пуллах, т.к. в массивах с fill-pointer-ом.
...
пулл тасков и общий пулл результатов (на данный момент есть третий общий ресурс:
переменная cnt, с помощью которой я нумерную склеенные картинки, см ~вставка~)

Алгоритм действия анализирующего потока:
~начало~
- если есть таски, взять один (предыдущее изображение, последнее изображение, список
y-point-ов)
- таск взят?
НЕТ!
- таски есть?
...
- сделать изображения битовыми для скорости
- НАЧАТЬ АНАЛИЗ: запустить цикл, который стработает столько раз, сколько у нас
  y-point-ов в списке
- в текущих результатах что-то есть?
НЕТ!
- analysis вызывается с порогом белых точек по умолчанию
- analsis вернул какой-то результат?
- если да, записываем его в список текущих результатов

ДА!
- analysis вызывается с порогом белых точек, который = кол-ву белых точек из послежнего
  результата
- analsis вернул какой-то результат?
- если да, записываем его в список текущих результатов (кол-во черных точек,
  кол-во белых точек и координату Y, которой этот результат получен)
ЦИКЛ ЗАКОНЧЕН
...
~вставка~
- склеить 2 картинки с помощью имеющегося результата
- сохранить склеенную картинку
~вставка окончена~
...
- пропадает первый таск, он берется потоком, анализируется, но склеенной картинки от
  него нет
- другой таск берется 2 раза, при этом не подряд, а с интервалом в несколько картинок,
  а создается этот таск только единожды. То есть в пулле тасков нет одинаковых тасков.
  Это видно на отладочном выводе из функции get-data.
...
На данный момент вместо вызова make-roll мы просто возвращаем t.
