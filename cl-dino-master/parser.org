...
#+NAME: make_y_points
#+BEGIN_SRC lisp :noweb yes
  (loop :for pnt :from 0 :to height-down :collect pnt)
#+END_SRC

Поскольку image-down накладывается на image-up, то мы будем использовать именно
координаты Y image-down. Они нам нужны снизу до верху.
