                        ...
                        (let ((alive-threads (alive-threads-counter thread-names)))
                          ...
                          ;; вызывать склейку
                              (progn
                                ...
                                (make-roll-test)
                                (return-from screen-scan t)) ;; TODO: поменять!!!
...
***** накладывание изображений с помощью xor: xor-area и append-xor
...
***** анализ изображений: analysis

Ну отксорили мы каждое изображение по энному поличеству раз. Что дальше?
...
Нам понадобится определить очередь:
...
Создадим процедуру, которая запускает один поток-поставщик и несколько
потоков-потребителей. Они разделяют две блокировки: ~task-queue-lock~ для
очереди (на ней выполняется ожидание) и ~outlock~ - для вывода сообщений
в стандартный вывод.

Это процедура возвращает список потоков-потребителей и обе блокировки
...
переменной ~task-queue-lock~, забирать ~task~ ,выполнять его и отправлять результат
выполнения в очередь результатов.
...
Для выполнения таска нам понадобится функция ~analize-img-pair~
Цель: наложить с помощью операции ксор ~image-down~ на ~image-up~ строка за строкой,
проанализировать наложение - т.е. посчитать сколько пикселей почернело из-за ксора -
собрать все результаты в список, где каждый подсписок представлен в виде
((кол-во черных пикселей . кол-во белых пикселей) . текущий y-point)) и вернуть список
результатов.
...
Если тасков нет (такое может случиться в самом начале сесии обработки) -
...
- выполнять задачи
- искать потоки по имени
- сообщать об остановке
ей нужны вспомогательные функции
