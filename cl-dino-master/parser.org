...
- запускает 3 анализирующих потока
- вызывает ~get-data~, которая скринит экран и делает таск на каждую пару изображений
- повторяет этот процесс до тех пор, пока не достигнем конца страницы выдачи (т.е. не
  встретим 2 одинаковых изображения)
- как только флаг последнего изображения ~last?~ = t, перестает вызывать ~get-data~
- ждет завершения анализирубщих потоков
...
~create-task~ сделает нам задания для 2-х изображений.  Задание выглядит
как заполненная структура task: массивы 2-х изображений и список точек
...
[COMMENT:gmm] Зачем нужны ~results~ и ~append-results~?
...
Функция подсчета живых потоков. Принимает список имен потоков, которые
надо проверить "на живость".
