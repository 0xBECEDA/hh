...
В структуре task добавилась возможность записать пути картинок для image-up и
image-down, это сделано для удобвства отладки.
...
#+BEGIN_SRC lisp :noweb yes
  ...
  (let ((tasks (make-array 100 :fill-pointer 0)) ;; зачем создавать здесь массив структур
        ;; с указателем заполнения, если можно было бы обойтись списком?
        (last? nil))
    (dotimes (i 100)
    ...
    (let ((results (make-array 100 :fill-pointer 0)))
      ...
      (let ((append-results (make-array 100 :fill-pointer 0)))
        ...
        ;; не make-tasks а create-task (и исправить везде где вызывается)
