...
они будут в пуллах. Пуллы представлены очередью с двумя указателями:
fill-pointer указывает на свободное место в пуле, а локальная переменная
cnt - на свободные таски, т.е.таски, которые не захвачены ни одним потоком
и их можно взять.
...
~начало~
- если есть необработанные таски, взять один (предыдущее изображение,
последнее изображение, список y-point-ов)
- таск взят?
НЕТ!
- таски есть?
нет!
           ...
           (with-open-file (out (format nil "thread~A" i) :direction :output
             ...
             (tagbody
              top
                ...
                (let* ((cur-task (if (> (fill-pointer tasks) cnt)
                                     (bt:with-lock-held (lock)
                                       (incf cnt)
                                       (aref tasks (- cnt 1))))))

                  ;; (format out "~% cnt ~A " cnt)
                  (format out "~%  f-p tasks ~A" (fill-pointer tasks))
                  ;; ОЖИДАЕМ ТАСКИ
                  (if (null cur-task)
                      (if (or (> cnt (fill-pointer tasks))
                              (eql cnt (fill-pointer tasks)))
                          (progn
                            (sleep 6)
                            ...
                            (let ((alive-threads 0))
                              ...
                              (if (eql 0 alive-threads)
                                  ...
                                  (progn
                                    ...
                                    (do ((i (fill-pointer tasks) (decf i)))
                                        (( = i 0))
                                      (vector-pop tasks))
          ...
          (tagbody
           append-images
             ...
             (let* (;; (cur-task (vector-pop results))
               ...
               ;; результаты аналза кончились?
               ;; (if (not (eql (fill-pointer results) 0))
               (if (not (eql counter (fill-pointer results)))
                   ;; нет!
                   (go append-images)
                   t)))))
