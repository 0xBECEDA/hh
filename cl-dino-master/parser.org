...
Это процедура возвращает список потоков-потребителей и обе блокировки (на
случай, если они нам понадобятся в будущем)
...
переменной ~task-queue-lock~, забирать ~task~, выполнять его и отправлять
результат выполнения в очередь результатов ~results-queue~.
...
Высота нового массива, куда будет сохранена отксоренная область наложения
теперь высчитывается как высота самой длинной картинки + ~y-point~. В
противном случае отксоренная область может не поместиться в созданный
массив.
