           ...
           (if last?
               ...
               (if (null cur-task)
                   ...
                   (if (null tasks)
                       ...
                       ;; я неправ
                       ;; Ответ: здесь нет никакого парадокса, таски генерит
                       ;; главный поток вместе со скринами,
                       ;; а анализирующий их только ждет и исполняет,
                       ;; так что если тасков
                       ;; не было 6 секунд назад, это не значит, что их не будет потом
                       (progn
                         (error 'my-test-error)
                         ;; таск появился!
                         ;; забираем
